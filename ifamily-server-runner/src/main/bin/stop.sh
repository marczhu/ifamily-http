...
if [ -z "$PIDPROC" ];then
 echo "ifamily-server-runner($INSTANCE_NAME_SYMBOL) is not running"
 exit 0
fi
...
do
if kill -9 $PID
   then echo "process ifamily-server-runner(Pid:$PID symbol:$INSTANCE_NAME_SYMBOL) was force stopped at " `date`
fi
done
echo stop finished.
